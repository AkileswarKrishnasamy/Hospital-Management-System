...
public interface BillRepostiory extends JpaRepository<Bill, Integer>{
	...
	@Modifying
	@Transactional
	@Query(value="DELETE FROM db.bill WHERE appointment_id IN (SELECT appointment_id FROM appointment WHERE doctor_id=?1)",nativeQuery = true)
	public int deleteByDoctorIdFromAppointment(Integer doctorId);
